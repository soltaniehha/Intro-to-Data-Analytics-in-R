...
```{r}
churnDf$SeniorCitizen <- ifelse(churnDf$SeniorCitizen==1, 'Yes', 'No')
```
...
```{r}
ggplot(churnDf, aes(x=Churn, fill = Churn)) +
  geom_bar(alpha = 2/3) +
  labs(y = "Count") +
  geom_text(aes(y = (..count..), 
                label = paste0(round(..count../sum(..count..)*100, 1), "%")), 
            stat = "count", color = "#230e0d", vjust = 1) + 
...
```{r}
multiplot(Churn_vs_var_plot(churnDf, "Dependents"),
          Churn_vs_var_plot(churnDf, "PhoneService"),
          Churn_vs_var_plot(churnDf, "MultipleLines"),
          Churn_vs_var_plot(churnDf, "InternetService"),
          layout=matrix(c(1,2,3,4),2,2,byrow=TRUE))
```
...
```{r}
ggplot(churnDf, aes(y = tenure, x = Churn, fill = Churn, 
                    color = Churn, alpha = 1/2)) + 
